...
async def connect_to_twitch_chat():
    await bot.connect()
    await bot.join(os.environ['CHANNEL_NAME']) # replace "CHANNEL_NAME" with the name of the channel the bot is connecting to 
    print(f"Bot {bot_nickname} has connected to the chat {os.environ['CHANNEL_NAME']}")
